...
const myBot = ['u6a66b6b389d27052f7c8ac0bdc2cee7e','u877ee5c8ad95d13040f004b0f60df6df','u9d1ef93a292bbc80dcdfd8bf13e10888'];
const banList = [];//Banned list
var groupList = new Array();//Group list
var vx = {};var midnornama,pesane,kickhim;var waitMsg = "no";//DO NOT CHANGE THIS
const imgArr = ['png','jpg','jpeg','gif','bmp','webp'];//DO NOT CHANGE THIS
var komenTL = "AutoLike by CyberTK"; //Comment for timeline
...
\n\n# Gunakan bot dengan bijak ^_^";
    ...
    poll(operation) {
		...
		if(operation.type == 5 && this.stateStatus.salam == 1) {//someone adding me..
            let halo = new Message();
			halo.to = operation.param1;
			halo.text = "Creator: line://ti/p/~cybertk0 (~CyberTK~)";
			this._client.sendMessage(0, halo);
        }
...
vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
		...
		if(vx[1] == "ban" && seq.from_ == vx[0] && waitMsg == "yes"){
			...
			if(txt == "cancel"){
				...
				if(isBanned(banList,pment)){
					waitMsg = "no";vx[0] = "";vx[1] = "";vx[2] = "";vx[3] = "";
					msg.text = cot[1]+"Success banlist";
		...
		if(vx[1] == "admin:on" && seq.from_ == vx[0] && waitMsg == "yes"){
			let panjang = txt.split("");
			...
			if(txt == "cancel"){
				...
				if(cot[1]){
					...
					if(isAdminOrBot(txt)){
						...
						let msg = new Message();msg.to = seq.to;msg.text = "";
						this._client.sendMessage(0, msg);
				        myBot.push(txt);
						waitMsg = "no";vx[0] = "";vx[1] = "";vx[2] = "";vx[3] = "";
					}
		...
		if(txt == "admin:on" && isAdminOrBot(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
				vx[2] = "arg1";
		...
		}else if(txt == "admin:on" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"Not permitted !");}
...
\n";
		...
		if(vx[1] == "left" && seq.from_ == vx[0] && waitMsg == "yes"){
			if(txt == "cancel"){
				vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
		...
		}else if(txt == "left" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"Not permitted !");}
		...
		if(vx[1] == "group" && seq.from_ == vx[0] && waitMsg == "yes"){
			if(vx[2]=="arg1"){
			let M = new Message();
			...
			switch(txt){
				...
				}else{this._sendMessage(seq,"");}
			}
		}
		if(txt == "group" && isAdminOrBot(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
				waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
		...
		}else if(txt == "group" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"Not permitted !");}
		
		if(cox[0] == "send" && isAdminOrBot(seq.from_) && cox[1]){
            let listMID = [];
        ...
        }else if(cox[0] == "send" && isAdminOrBot(seq.from_) && !cox[1]){this._sendMessage(seq,"");}else if(cox[0] == "send" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"Not permitted!");}
		
		if(txt == "/^[A-Za-z0-9 ]+$/" && seq.toType == 2  && this.stateStatus.kick == 1){
			this._sendMessage(seq,"Ok Kick..!");
        ...
        if(txt === 'şu','siz','biz','o','ben','sen','of','efendim','kanka','avcı','hakan','ilizyon','zafer','tolga','hayır','evet','yani','hehe','haha','hahaha','zuhaha','tamam','nasilsiniz','nasılsınız','nasilsin,','nasılsın','selam','selamunaleykum','selam','selamunaleykum','sa','as','slm','merhaba','amk','kaan','dogan','habibo' && this.stateStatus.kick == 1 && seq.toType == 2) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
        ...
        if(txt === 'Fazla zıplamayin birdbrainler','of' && this.stateStatus.kick == 1 && seq.toType == 2) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
                if(!isAdminOrBot(listMember[i].mid)){
                    this._kickMember(seq.to,[listMember[i].mid])
...
\n# Group Cover:\nhttp://dl.profile.line.naver.jp/"+gcover;
		...
		if(gTicket[0] == "join" && isAdminOrBot(seq.from_)){
			...
			if(sudah == "ya"){
				let bang = new Message();bang.to = seq.to;bang.text = "";
				this._client.sendMessage(0,bang);
			}else if(sudah == "no"){
